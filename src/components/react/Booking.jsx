...
const Booking = ({ dataCategory, dataUrl }) => {
    ...
    return (
        <>
            <div className='group'>
                <div className={`bg-primary-orange transition-colors duration-500 ease-in-out ${dataCategory && dataUrl && isOpen && 'bg-primary-purple'} ${dataCategory && dataUrl && 'group-hover:bg-primary-purple'}`} data-push='right'>
                    <button className={`bg-primary-orange text-white font-medium flex items-center gap-5 justify-between sm:gap-[4.75rem] px-5 lg:px-11 py-3 min-h-[5rem] sm:min-h-[6.25rem] w-full transition-colors duration-500 ease-in-out ${dataCategory && dataUrl && isOpen && 'bg-primary-purple'} ${dataCategory && dataUrl && 'group-hover:bg-primary-purple'}`} onClick={toggleDropdown}>
                        <span className='text-3xl font-medium'>{dataCategory && dataUrl ? 'Check availability' : 'Booking unavailable'}</span>
                        {dataCategory && dataUrl && isOpen &&
