        ...
        <div className={`fixed h-[calc(100dvh-7rem)] left-0 top-[7rem] w-full bg-off-white p-10 text-2xl font-medium z-50 duration-500 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            <ul className='flex flex-col gap-5'>
              {items.items.map((item, i) =>(
                <li key={i}>
                    <a class="hover:underline focus:underline" href={item.url}>
                        {item.title}
                    </a>
                </li>
              ))}
            ...
            <span>Book now</span>
            <span class="group-hover:rotate-45 duration-500 ease-in-out block w-4 h-4 undefined">
                  <svg viewBox="0 0 17.978 17.978" astro-icon="arrow"><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="m1.414 16.564 15.15-15.15" data-name="Line 1"></path><path d="M1.414 1.414h15.18v15.18" data-name="Path 1"></path></g></svg>
                </span>
          </a>
        </div>
